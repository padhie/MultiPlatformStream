    ...
    public function indexAction(Request $request)
    {
    	...
    	if ($request->get("submit") == "1") {
			...
			if (strlen($streamString) > 0) {
				...
				$streamString = array_unique($streamString);
				dump($streamString);
		...
		foreach ($streamParameter AS $streamanem) {
			if ($streamanem != "") {
				...
				if ($explodeStream[0] == "yt") {
					...
					if (isset($apiData->id)) {
						$streams = $youtube->searchChannelLiveStream($explodeStream[1], $apiData->id);
						if (isset($streams[0]) && isset($streams[0]->id) && isset($streams[0]->id->videoId)) {
							$streamList[] =  [
								"source" => "youtube",
								"row" => $explodeStream[1],
								"channel" => $apiData->id,
								"stream" => "https://www.youtube.com/embed/live_stream?channel=".$apiData->id,
